...
import core, { API, ASTPath, Collection, FileInfo, TaggedTemplateExpression } from 'jscodeshift'

// jscodeshift can take a parser, like "babel", "babylon", "flow", "ts", or "tsx"
// Read more: https://github.com/facebook/jscodeshift#parser
export const parser = 'flow'

function addMissingTypeImport(root: Collection<any>, j: core.JSCodeshift, importTypeName: string) {
...
function extractQueryNameFromTemplateLiteral(
    ...
    const queryNameResult =
        queryTemplateLiteral?.quasi?.quasis[0].value.raw.match(/query (?<queryName>[^ ]+)/)
    const queryName =
...
export default function (file: FileInfo, api: API) {
    ...
    useLazyLoadQueries.forEach((useLazyLoadQueryPath) => {
        ...
        const addGenericAndMissingImport = (
            ...
            const queryName = extractQueryNameFromTemplateLiteral(queryTemplateLiteralString)

            // Add generic to useLazyLoadQuery hook
            const genericsToSpecify = j.typeParameterInstantiation([])
            const genericQueryType = j.genericTypeAnnotation(j.identifier(queryName), null)
        ...
        } else if (useLazyLoadQueryNode.arguments[0].type === 'TaggedTemplateExpression') {
